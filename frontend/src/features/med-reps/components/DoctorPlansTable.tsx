                                ...
                                <div className="grid gap-2">
                                    ...
                                    <Select value={doctorId} onValueChange={setDoctorId}>
                                        ...
                                        <SelectContent className="rounded-xl border-slate-200 shadow-xl max-h-[200px]">
                                            {doctors.map((doctor) => (
                                                <SelectItem key={doctor.id} value={doctor.id.toString()} className={`font-medium ${doctor.is_active === false ? 'text-slate-400 opacity-70' : ''}`}>
                                                    {doctor.full_name} {doctor.is_active === false && "(Faol emas)"} <span className="text-slate-400 text-xs ml-2">({doctor.specialty?.name})</span>
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
