...
import { getWarehouseUserColumns } from './WarehouseUserColumns';
import { CreateWarehouseUserModal } from './components/CreateWarehouseUserModal';
import { EditManagerModal } from '../product-managers/EditManagerModal';

export default function WarehouseUserManagementPage() {
    ...
    const { data: managers, isLoading, refetch } = useQuery<User[]>({
        ...
        queryFn: async () => {
            ...
            return response.data.filter((u: User) => u.role === UserRole.HEAD_OF_WAREHOUSE);
        },
    });

    const columns = React.useMemo(() => getWarehouseUserColumns(
    ...
    return (
        <PageContainer>
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Завсклады</h1>
                    <p className="text-slate-500 mt-1 font-medium">Управление заведующими складом и их доступом</p>
                </div>
                <Button
