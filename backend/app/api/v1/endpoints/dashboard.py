...
async def get_dashboard_stats(
    ...
    try:
        ...
        if current_user.role == UserRole.REGIONAL_MANAGER:
            # RELATIONSHIP CHECK: Ensure assigned_regions is pre-loaded or handles None
            try:
                allowed_region_ids = [r.id for r in current_user.assigned_regions]
            except Exception as e:
                # Handle cases where relationship might not be initialized properly
