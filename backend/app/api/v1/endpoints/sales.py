...
async def delete_payment(
    *,
    ...
    # Per user request: Only accountant and owner (Investor/Admin/Director) can delete
    if current_user.role not in [UserRole.ACCOUNTANT, UserRole.INVESTOR, UserRole.ADMIN, UserRole.DIRECTOR]:
    ...
    for rep in medreps:
        ...
        for ge in global_entries:
            if ge.ledger_type == LedgerType.ACCRUAL:
                ...
                if ge.is_paid:
                    g_paid += ge.amount
            elif ge.ledger_type in [LedgerType.ADVANCE, LedgerType.PAYOUT]:
                # Treat as both earned and paid to avoid artificial predinvest
                g_accrued += ge.amount
                g_paid += ge.amount
            elif ge.ledger_type == LedgerType.OFFSET:
                g_paid += ge.amount
